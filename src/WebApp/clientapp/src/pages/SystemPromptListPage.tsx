...
import { systemPrompts } from '@/lib/callCenterData'
            ...
            <div key={prompt.id} className="rounded-xl border bg-background p-4">
              <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                <div>
                  ...
                  <p className="mt-2 text-sm text-muted-foreground">{prompt.type} / {prompt.version}</p>
                  <p className="mt-1 text-sm text-muted-foreground">{prompt.content}</p>
                </div>
                <div className="flex items-center gap-3">
                  <span className={cn('rounded-full px-3 py-1 text-xs font-medium', prompt.enabled ? 'bg-emerald-100 text-emerald-800' : 'bg-secondary text-secondary-foreground')}>
                    {prompt.enabled ? '有効' : '無効'}
                  </span>
                  <Link to={`/system-prompts/${prompt.id}`} className={cn(buttonVariants({ size: 'sm', variant: 'outline' }))}>
                    詳細
                  </Link>
                </div>
              </div>
