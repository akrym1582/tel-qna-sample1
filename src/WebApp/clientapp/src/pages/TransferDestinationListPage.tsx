...
import { transferDestinations } from '@/lib/callCenterData'
            ...
            <div key={destination.id} className="rounded-xl border bg-background p-4">
              <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                <div>
                  ...
                  <p className="mt-2 text-sm text-muted-foreground">{destination.department} / {destination.type}</p>
                  <p className="mt-1 text-sm text-muted-foreground">ヒント: {destination.hint}</p>
                </div>
                <div className="flex items-center gap-3">
                  <span className={cn('rounded-full px-3 py-1 text-xs font-medium', destination.enabled ? 'bg-emerald-100 text-emerald-800' : 'bg-secondary text-secondary-foreground')}>
